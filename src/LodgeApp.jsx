...
/* ─── MAIN APP ───────────────────────────────────────────── */
export function LodgeApp({ tenant, onLogout }) {
  // Tenant-scoped API wrapper
  const tid = tenant?.id;
  const api = {
    getLocations:   ()     => rawApi.getLocations(tid),
    createLocation: d      => rawApi.createLocation(tid, d),
    updateLocation: (id,d) => rawApi.updateLocation(tid, id, d),
    deleteLocation: id     => rawApi.deleteLocation(tid, id),
    getRooms:       lid    => rawApi.getRooms(tid, lid),
    createRoom:     d      => rawApi.createRoom(tid, d),
    updateRoom:     (id,d) => rawApi.updateRoom(tid, id, d),
    deleteRoom:     id     => rawApi.deleteRoom(tid, id),
    getBookings:    lid    => rawApi.getBookings(tid, lid),
    createBooking:  d      => rawApi.createBooking(tid, d),
    updateBooking:  (id,d) => rawApi.updateBooking(tid, id, d),
    recordPayment:  (id,a) => rawApi.recordPayment(tid, id, a),
    extendBooking:  (id,d) => rawApi.extendBooking(tid, id, d),
    deleteBooking:  id     => rawApi.deleteBooking(tid, id),
    getExpenses:    lid    => rawApi.getExpenses(tid, lid),
    createExpense:  d      => rawApi.createExpense(tid, d),
    getStaff:       ()     => rawApi.getStaff(tid),
    createStaff:    d      => rawApi.createStaff(tid, d),
    updateStaff:    (id,d) => rawApi.updateStaff(tid, id, d),
    updateProfile:  d      => rawApi.updateProfile(tid, d),
    getReports:     lid    => rawApi.getReports(tid, lid),
    login:          (e,p)  => rawApi.staffLogin(tid, {email:e, pin:p}),
    customerLogin:  d      => rawApi.custLogin(tid, d),
    customerRegister: d    => rawApi.custRegister(tid, d),
    customerBookings: cid  => rawApi.custBookings(tid, cid),
    customerUpdate:  (cid,d) => rawApi.custUpdate(tid, cid, d),
    customerCancel:  (id,cid) => rawApi.custCancel(tid, id, cid),
  };
